...
function highlightStar(obj,id) {
	removeHighlight(id);		
	$('.demo-table #skills-'+id+' li').each(function(index) {
		$(this).addClass('highlight');
		if(index == $('.demo-table #skills-'+id+' li').index(obj)) {
			return false;	
		}
	});
}
...
function removeHighlight(id) {
	$('.demo-table #skills-'+id+' li').removeClass('selected');
	$('.demo-table #skills-'+id+' li').removeClass('highlight');
}

function addRating(obj,id) {
	$('.demo-table #skills-'+id+' li').each(function(index) {
		$(this).addClass('selected');
		$('#skills-'+id+' #rate').val((index+1));
		if(index == $('.demo-table #skills-'+id+' li').index(obj)) {
			return false;	
		}
	});
	$.ajax({
	url: "add_rating.php",
	data:'id='+id+'&rate='+$('#skills-'+id+' #rate').val(),
	type: "POST"
	});
}

function resetRating(id) {
	if($('#skills-'+id+' #rate').val() != 0) {
		$('.demo-table #skills-'+id+' li').each(function(index) {
			$(this).addClass('selected');
			if((index+1) == $('#skills-'+id+' #rate').val()) {
				return false;	
			}
		});
	}
